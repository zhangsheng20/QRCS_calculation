%calculate Circumferential scattering of sphere

function[l_NIs]= calculate_sphere_back_G(filename,frequency_G,step,theta_begin,theta_end,phi_begin,phi_end,fileformat,Is_plot)
global nf;
global fr0;
global fn;
global fA;
...
if (nargin==0)
    filename='C:\Users\Administrator\Desktop\quantum stealth\mygit\model\sphere-r-0.1\sphere-r-0.1-grid-0.001-gridpro.dat';
end
...
if(theta_begin~=theta_end)
    l_theta_d=theta_begin:step:theta_end;
    l_sigma_Q_1=zeros(size(l_theta_d));
    phi=phi_begin;
    ii=0;
    for theta= l_theta_d
        ii=ii+1;
        ...
        fprintf('progress: %d \\ %d  A:%f   G:%f  G/A^2:%f  G/A:%f  \n', ...
                          ii,length(l_theta_d),A,G,G/A^2,G/A);
...
elseif(phi_begin==phi_end && theta_begin==theta_end)
    ...
    l_theta_d=0:0.01:pi;
    l_NIs=zeros(size(l_theta_d));
    for ii=1:length(l_theta_d)
        theta_d=l_theta_d(ii);
        l_NIs(ii)=calc_3D_N_I_s_3(0,0,theta_d,0);
    end  
    figure;
    plot(l_theta_d,10*log10(l_NIs),'-')
    grid on;
end
...
fprintf('theta_d:%f S:%f  %f \n',theta_d,sum(fA.*mask),sum(fA.*mask)/(pi-theta_d));
...
for ii=1:1:nf
    if (dot(r_s,fn(ii,:))/r>0) &&  (dot(r_d,fn(ii,:))/r>0) %�����䵽�Ĺ���
...
for ii=1:1:nf 
  ...
  b=dot(r_d,fn(ii,:))/r;
  %fprintf(' %d:a=%f,b=%f \n',ii,a,b); 
   if (a>0.1) &&  (b>0.1)  && fn(ii,3)>0.5         %�����䵽��ԭ��
...
%% 
    ...
    vTmp=cell(1,np);
    fTmp=cell(1,np);
    nvTmp=zeros(1,np); %v�����ڵ�
    nfTmp=zeros(1,np); %f������Ԫ
    for iPart=1:np
    ...
    nf=size(facet,1);
    nv=size(vertex,1);
    vertex=cat(3,vertex(facet(:,1),:),vertex(facet(:,2),:),vertex(facet(:,3),:));%point (��Ԫ��ţ�x/y/z���ڼ�����) 
    clear vTmp;
    clear fTmp;
    clear nXTmp;
    clear nFTmp;
